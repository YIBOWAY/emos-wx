<template>
	<view>
		...
		<image mode="widthFix" :src="photoPath" v-if="showImage" class="image"></image>
	...
	export default {
		...
		onLoad: function() {
			qqmapsdk = new QQMapWX({
				key:"IOJBZ-JMOKX-JFY46-TMTQJ-ABVQK-VGBP4" //腾讯位置服务的key
			})
		},
		onShow:function(){
			let that = this
		...
		methods: {
			clickBtn:function(){
				...
				if(that.btnText == '拍照'){
					...
					uni.getLocation({ //获取地址，经纬度坐标
						type:"wgs84",
						success:function(resp){
							let latitude = resp.latitude //经度数据
							let longitude = resp.longitude //纬度数据
							console.log(latitude)
							console.log(longitude)
							qqmapsdk.reverseGeocoder({//将坐标提供给腾讯位置服务以获得详细地址信息
								location:{
									latitude:latitude,
									longitude:longitude
								},
								success:function(resp){
									...
									let addressComponent = resp.result.address_component; //返回的json数据格式
